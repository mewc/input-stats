...
- Pointer events also carry the screen they happened on (`events.display` → `displays`, keyed by display UUID). `DisplayResolver` caches screen frames flipped into CGEvent coordinate space; keystrokes stay unattributed because the pointer is often on a different screen than the focused window.
...
- **Tiered retention:** 5-second rows are folded into one-minute buckets after 7 days (`compactLocked`), then everything is pruned at 30 days. The drilldown only offers 5s blocks for spans ≤1h, which are always recent, so nothing reachable is lost — and minute sums are preserved exactly, so already-uploaded cloud minutes can never conflict.
- **Nothing on the input path or launch may touch SQLite synchronously.** `EventStore` opens/migrates/compacts on its own queue; device and screen resolution happen in the background and attribute to the unknown row until they land.
